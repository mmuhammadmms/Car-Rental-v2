 ...
 ?>
    ...
    <div class="container">
                  ...
                  $cid = $_GET['id'];
                    ...
                    $strSQL3 = "SELECT FILESTYPE from CARSIMAGE where cars_id = '$cid' and imgstatus != 'Disabled'";
      ...
      ?>
        ...
        <div class="row">
                ...
                <h1 class="page-header"><?php echo $objResult["CARS_NAME"];?><br />
                    <small>Available at : <?php echo $objResult["CARS_LOCATION"];?> </small>
                </h1>
        ...
        <div class="row">
            ...
            $strSQL = "select COALESCE(AVG(feedback_rating), 0)  AS AV from feedback F , reservation R , cars C where F.reservation_id = R.reservation_id AND R.cars_id = C.cars_id AND C.cars_id = '$cid'";
          	$objParse = oci_parse($conn, $strSQL);
          	$run = oci_execute ($objParse,OCI_DEFAULT);
            ...
            <div align="right">
            <input id="rating-input"  value="<?php echo $objResult['AV'];?>" type="text" class="rating" data-min=0 data-max=5 data-step=0.1 data-size="sm"
    ...
    include ("connect.php");
    $strSQL3 = "
SELECT TO_CHAR(TO_DATE(pickup_date, 'DD/MM/YYYY'),'YYYY-MM-DD') AS pdate , TO_CHAR(TO_DATE(return_date, 'DD/MM/YYYY'),'YYYY-MM-DD') AS rdate FROM RESERVATION WHERE CARS_ID = '$cid' AND RESERVATION_STATUS NOT IN ('Canceled' , 'Pending' , 'Rejected' )";
    $objParse3 = oci_parse($conn, $strSQL3);
    oci_execute ($objParse3,OCI_DEFAULT);

    $strSQL2 = "select COUNT(*) AS COUNT FROM (SELECT TO_CHAR(TO_DATE(pickup_date, 'DD/MM/YYYY'),'YYYY-MM-DD') AS pdate , TO_CHAR(TO_DATE(return_date, 'DD/MM/YYYY'),'YYYY-MM-DD') AS rdate FROM RESERVATION WHERE CARS_ID = '$cid' AND RESERVATION_STATUS NOT IN ('Canceled' , 'Pending' , 'Rejected' ) )";
    $objParse2 = oci_parse($conn, $strSQL2);
    oci_execute ($objParse2,OCI_DEFAULT);
    ...
    var unavailableDates = [
      ...
      { ?>
          {start: '<?php echo $objResult3[0];?>', end: '<?php echo $objResult3[1];?>'},
      <?php
      } ?>
